...
import type { AddFavoriteInput, Favorite, FavoriteKind } from "../shared/index";
import { favoriteKey } from "../shared/index";
...
export function LibraryProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    libraryApi
      .getFavorites()
      .then((data) => {
        if (isMounted) setFavoritesState({ userId, items: data });
      })
      .catch(() => {
        if (isMounted) setFavoritesState({ userId, items: [] });
      });
